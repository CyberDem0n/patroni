...
def create_distributed_table(context, name):
    context.pctl.query(name, 'CREATE TABLE d(id int not null)')
...
def insert_thread(query_func, context):
    while True:
        ...
        query_func('INSERT INTO d VALUES({0})'.format(context.insert_counter))
...
def count_rows(context, name):
    rows = context.pctl.query(name, "SELECT COUNT(*) FROM d").fetchone()[0]

...
from ..utils import deep_compare, enable_keepalive, iter_response_objects, parse_int, RetryFailedError, USER_AGENT
...
class Etcd3(AbstractEtcd):
    ...
    def _citus_cluster_loader(self, path):
        ...
        for node in self._client.get_cluster(path):
            ...
            if len(key) == 2 and parse_int(key[0]) is not None:
                clusters[key[0]][key[1]] = node
